...
public class BeeWanderState : BeeBaseState
{
    ...
    public BeeWanderState(Bee b) : base(b.gameObject)
    {
        bee = b;
    }
    ...
    private Transform CheckForAggro()
    {
        ...
        Collider[] rangeChecks = Physics.OverlapSphere(pos, BeeEnemySettings.AggroRadius, BeeEnemySettings.TargetMask);
        ...
        if (rangeChecks.Length != 0)
        {
            ...
            if (Vector3.Angle(transform.forward, directionToTarget) < BeeEnemySettings.Angle / 2)
            {
                float distanceToTarget = Vector3.Distance(pos, target.position);

                if (Physics.Raycast(pos, directionToTarget, out hit, BeeEnemySettings.AggroRadius))
                {
